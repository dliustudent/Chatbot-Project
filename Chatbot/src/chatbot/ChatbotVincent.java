...
public class ChatbotVincent implements Topic {
	...
	public void talk(String response) {	
			...
			for(int i = 0 ; i < actualKey.length ; i++) {
				if(ChatbotMain.findKeyword(response, secretKeyword, 0) >= 0) {
					...
					if(response.equals(goodbyeKeyword)) {
						...
						ChatbotMain.chatbot.returnChatting("Mammal","Normal");
					}
					i = -1;
				}
			}
		...
		if(finished == true) {
			ChatbotMain.print("Well it was nice talking to you, "+ ChatbotMain.chatbot.getUsername()+"! I'll send you to the other bots now!");
			ChatbotMain.chatbot.returnChatting("Mammal","Mad");
		}
	...
	public void checkIfAnimalIsLikedByUser(String anim, String desc,String b) {
		lastTalk = anim;
		ChatbotMain.print(desc);
		response = ChatbotMain.getInput();
		...
		while(continueWhile == true) {
			if(handleCondition(b)) {
				...
				if(ChatbotMain.findKeyword(b, "Lion", 0) >= 0) {
					checkIfAnimalIsLikedByUser("Lion", "Lions are the king of the Savannas. Do you like them!",b);
				}else if (ChatbotMain.findKeyword(b, "Elephant", 0) >= 0) {
					checkIfAnimalIsLikedByUser("Elephant", "Elephants are the largest land animals in the world. Do you like them!",b);
				}
				else if (ChatbotMain.findKeyword(b, "Panda", 0) >= 0) {
					checkIfAnimalIsLikedByUser("Panda","Pandas are black and white bears. In the wild, they are found in thick bamboo forests, high up in the mountains of central China. Do you like them!",b);
				}else if (ChatbotMain.findKeyword(b, "dog", 0) >= 0) {
					checkIfAnimalIsLikedByUser("Dog","A dogs sense of smell is more than 1 million times stronger than that of a person. Do you like them!",b);
				}else if (ChatbotMain.findKeyword(b, "cat", 0) >= 0) {
					checkIfAnimalIsLikedByUser("cat","Domestic cats love to play, this is especially true with kittens who love to chase toys and play fight. Play fighting among kittens may be a way for them to practice and learn skills for hunting and fighting. Do you like them!", b);
				}else if (ChatbotMain.findKeyword(b, "gorilla", 0) >= 0) {
					checkIfAnimalIsLikedByUser("Gorilla","Gorillas are ground-dwelling, predominantly herbivorous apes that inhabit the forests of central Africa. Do you like them!",b);
				}
				else {
					continueWhile = false;
				}
